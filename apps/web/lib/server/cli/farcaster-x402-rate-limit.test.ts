...
import { enforceCliFarcasterX402RateLimit } from "./farcaster-x402-rate-limit";

describe("enforceCliFarcasterX402RateLimit", () => {
  beforeEach(() => {
    vi.clearAllMocks();
    process.env.CLI_FARCASTER_X402_RATE_LIMIT_PER_MINUTE = "60";
    process.env.CLI_FARCASTER_X402_MAX_CALLS_PER_DAY = "10000";
  });

  afterEach(() => {
    delete process.env.CLI_FARCASTER_X402_RATE_LIMIT_PER_MINUTE;
    delete process.env.CLI_FARCASTER_X402_MAX_CALLS_PER_DAY;
  });
  ...
  it("uses atomic kv.incr windows and allows requests within limits", async () => {
    ...
    const result = await enforceCliFarcasterX402RateLimit({
      request,
  ...
  it("returns daily cap errors when the token daily limit is exceeded", async () => {
    process.env.CLI_FARCASTER_X402_MAX_CALLS_PER_DAY = "2";
    kvIncrMock
