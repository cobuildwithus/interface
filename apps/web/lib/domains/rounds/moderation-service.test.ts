...
describe("markIneligible", () => {
  ...
  it("defaults to farcaster source and normalizes cast hashes", async () => {
    const { markIneligible } = await import("./moderation-service");

    const res = await markIneligible({
      userAddress: `0x${"c".repeat(40)}` as const,
      ruleId: 7,
      castHash: "A".repeat(40),
      moderatorNote: "spam",
      castText: "cast",
    ...
    expect(res).toEqual(
      expect.objectContaining({ ok: true, removedSubmission: true, affectedRoundIds: ["1"] })
    );
    expect(mockRoundSubmissionDeleteMany).toHaveBeenCalledWith(
      expect.objectContaining({
        where: expect.objectContaining({
          source: "farcaster",
          postId: { in: ["0xaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaa"] },
        }),
      })
    );
  });

  it("retries rewrite and persists requirements on second-attempt success", async () => {
    mockRewriteRequirementsText
      .mockRejectedValueOnce(new Error("retryable"))
      .mockResolvedValueOnce("rewritten requirements");

    const { markIneligible } = await import("./moderation-service");

    const res = await markIneligible({
      userAddress: `0x${"d".repeat(40)}` as const,
      ruleId: 7,
      source: "x",
      castHash: "1234567890",
      moderatorNote: "clarify rule",
      castText: "x post",
      alsoUpdateRequirements: true,
    });

    expect(res).toEqual(expect.objectContaining({ ok: true, requirementsUpdated: true }));
    expect(mockRewriteRequirementsText).toHaveBeenCalledTimes(2);
    expect(mockPostFilterRuleUpdate).toHaveBeenCalledWith(
      expect.objectContaining({
        where: { id: 7 },
        data: { requirementsText: "rewritten requirements" },
      })
    );
  });

  it("handles async rescore failures without failing moderation", async () => {
    const consoleErrorSpy = vi.spyOn(console, "error").mockImplementation(() => undefined);
    mockRescoreOpenRoundsForRule.mockRejectedValueOnce(new Error("rescore-failed"));

    const { markIneligible } = await import("./moderation-service");

    const res = await markIneligible({
      userAddress: `0x${"e".repeat(40)}` as const,
      ruleId: 7,
      source: "x",
      castHash: "1234567890",
      moderatorNote: "spam",
    ...
    expect(res).toEqual(expect.objectContaining({ ok: true, removedSubmission: true }));
    expect(consoleErrorSpy).toHaveBeenCalledWith(
      "[moderation] Failed to trigger rescore for rule 7:",
      expect.any(Error)
    );
