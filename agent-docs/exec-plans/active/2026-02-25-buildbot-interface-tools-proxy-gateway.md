...
Implement interface-side proxy endpoints for Cli CLI docs/tools calls so:
...
- New POST endpoints exist:
  - `/api/docs/search`
  - `/api/cli/tools/get-user`
  - `/api/cli/tools/get-cast`
  - `/api/cli/tools/get-treasury-stats`
- All endpoints require `requireCliBearerAuth`.
...
- New interface route handlers under `apps/web/app/api/docs/search` and `apps/web/app/api/cli/tools/**`.
- Shared server-only cli tools proxy helper for validation, rate limit, and chat-api pass-through.
...
- Changes to existing cli exec/wallet storage flow currently owned by another active ledger row.
...
3. Risk: overlap with active sibling work.
   Mitigation: keep strict file/symbol boundaries and avoid touching `cli/exec` + `wallet-store`.

## Planned changes

1. Add `lib/server/CLI/tools-proxy.ts` with:
...
2. Add route handlers:
   ...
   - `app/api/cli/tools/get-user/route.ts`
   - `app/api/cli/tools/get-cast/route.ts`
   - `app/api/cli/tools/get-treasury-stats/route.ts`
