...
export async function enforceCliFarcasterX402RateLimit(params: {
  request: Request;
  ownerAddress: `0x${string}`;
  sessionId: string;
  agentKey: string;
}): Promise<FarcasterX402RateLimitResult> {
  const perMinute = parseLimit("CLI_FARCASTER_X402_RATE_LIMIT_PER_MINUTE", DEFAULT_PER_MINUTE);
  const perDay = parseLimit("CLI_FARCASTER_X402_MAX_CALLS_PER_DAY", DEFAULT_PER_DAY);
  ...
  try {
    const [principalMinute, sessionMinute, ipMinute, sessionDaily] = await Promise.all([
      checkWindow({
        keyPrefix: "cli:farcaster:x402:minute:principal",
        rawKey: principalKey,
        nowMs,
      ...
      checkWindow({
        keyPrefix: "cli:farcaster:x402:minute:session",
        rawKey: sessionKey,
        nowMs,
        windowSeconds: MINUTE_WINDOW_SECONDS,
        maxCount: perMinute,
      ...
      checkWindow({
        keyPrefix: "cli:farcaster:x402:minute:ip",
        rawKey: ip,
        nowMs,
        windowSeconds: MINUTE_WINDOW_SECONDS,
        maxCount: perMinute,
      ...
      checkWindow({
        keyPrefix: "cli:farcaster:x402:daily:session",
        rawKey: sessionKey,
        nowMs,
    ...
    if (!sessionDaily.allowed) {
      return {
        allowed: false,
        status: 429,
        error: DAILY_CAP_ERROR,
        retryAfterSeconds: Math.max(1, Math.ceil((sessionDaily.resetAtMs - nowMs) / 1000)),
      };
    }

    if (!principalMinute.allowed || !sessionMinute.allowed || !ipMinute.allowed) {
      const resetAtMs = Math.max(
        principalMinute.resetAtMs,
        sessionMinute.resetAtMs,
        ipMinute.resetAtMs
      );
      return {
        allowed: false,
    ...
    console.error("[cli][farcaster][x402-payment] rate limit failed", error);
    return {
      allowed: false,
      status: 503,
