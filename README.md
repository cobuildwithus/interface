...
`cd apps/contracts && pnpm generate` runs `wagmi generate` and then syncs:
...
pnpm --filter contracts generate # Regenerate contract bindings and sync web ABIs/addresses
...
- If you need a non-conventional message, pass `--allow-non-conventional` (or set `COMMITTER_ALLOW_NON_CONVENTIONAL=1`).
