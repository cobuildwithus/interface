...
describe("createThreadPost", () => {
  ...
  it("treats non-object payloads as invalid input", async () => {
    const result = await createThreadPost("not-json-object");
  ...
  it("uses default publish error details when status/error are missing", async () => {
    getSession.mockResolvedValueOnce({ farcaster: { fid: 10 } });
    getSignerRecord.mockResolvedValueOnce({ signerPermissions: ["cast"], signerUuid: "uuid" });
    hasCastPermission.mockReturnValueOnce(true);
    neynarPublishCast.mockResolvedValueOnce({ ok: false });
  ...
  it("includes both attachment and embed urls when distinct", async () => {
    getSession.mockResolvedValueOnce({ farcaster: { fid: 10 } });
    getSignerRecord.mockResolvedValueOnce({ signerPermissions: ["cast"], signerUuid: "uuid" });
    hasCastPermission.mockReturnValueOnce(true);
    ...
    const result = await createThreadPost({
      ...
      embedUrl: "https://b.com",
    });

    expect(neynarPublishCast).toHaveBeenCalledWith(
      expect.objectContaining({
        embeds: [{ url: "https://a.com" }, { url: "https://b.com" }],
        retriesOn429: 8,
      })
    );
  ...
  it("returns success even when upsert is skipped after publish", async () => {
    const consoleWarn = vi.spyOn(console, "warn").mockImplementation(() => {});
    getSession.mockResolvedValueOnce({ farcaster: { fid: 10 } });
    getSignerRecord.mockResolvedValueOnce({ signerPermissions: ["cast"], signerUuid: "uuid" });
    hasCastPermission.mockReturnValueOnce(true);
    neynarPublishCast.mockResolvedValueOnce({ ok: true, hash: "hash-1" });
    upsertCobuildCastByHash.mockResolvedValueOnce(false);

    const result = await createThreadPost({ title: "hello", content: "world" });

    expect(consoleWarn).toHaveBeenCalledWith("[farcaster/thread] upsert skipped after publish", {
      hash: "hash-1",
      index: 0,
    });
    expect(revalidateTag).toHaveBeenCalledWith("discussion", "seconds");
    expect(revalidateTag).toHaveBeenCalledWith("thread", "seconds");
    expect(result).toEqual({ ok: true, data: { hash: "hash-1" } });
  });
