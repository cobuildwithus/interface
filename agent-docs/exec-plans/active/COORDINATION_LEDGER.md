...
| Codex / goal-create-refactor | Replace local goal-create payload/decode logic with shared wire helpers                 | `apps/web/app/(app)/goals/create/**`, matching `agent-docs/**` notes                                                                                                                                                                  | Refactor `CreateGoalForm` input parsing/build/decode flow around shared wire goal-create builders and typed receipt decoding; add explicit spend-policy inputs if canonical defaults are unavailable | Depends on ABI-correct `wire` goal-create helpers; keep wallet/auth transaction flow unchanged and avoid chat/notification files | 2026-03-10           |
