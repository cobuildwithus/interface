# CLI CLI + CDP server wallet integration (Privy identity)
...
- Add a sibling CLI package `../CLI` that calls the interface app over HTTP.
- Add server-side `CLI` API routes in `apps/web` that map Privy wallet identity to one CDP server wallet account per `(ownerAddress, agentKey)`.
...
- `POST/GET/DELETE /api/CLI/token` (session-authenticated PAT management)
- `POST /api/CLI/wallet` (Bearer PAT, get/create wallet)
- `POST /api/CLI/exec` (Bearer PAT, `transfer` or `tx`)
...
- `POST /api/CLI/exec` supports idempotency keys (`Idempotency-Key` header or body field) with DB-backed replay lookup.
- Strict policy mode (`CLI_STRICT=1`) can fail closed when required allowlists/caps are missing.
- Optional CDP account policy attachment is supported via `CLI_ACCOUNT_POLICY_ID`.
- New sibling package `../CLI` runs CLI commands for config, wallet fetch, transfer, and tx.
...
- In scope:
- Prisma schema additions for CLI token/wallet/log records.
...
- Keep API naming `CLI` (not `broker`) for clarity.
...
2. Implement `apps/web/lib/server/CLI/*` modules.
3. Add `apps/web/app/api/CLI/*` routes.
4. Add tests for auth/policy/route behavior.
5. Create sibling CLI package `../CLI` with command wiring.
